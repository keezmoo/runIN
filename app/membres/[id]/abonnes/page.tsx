...
export default async function AbonnesPage({ params }: PageProps) {
  ...
  const { data: suivis } = await supabase
    ...
    .eq("profil_suivi_id", id)
    .order("created_at", {
      ascending: false,
    });

  const idsProfils = suivis?.map((suivi) => suivi.utilisateur_id) ?? [];
  ...
  return (
    <main className="mx-auto max-w-xl p-6">
      ...
      <h1 className="mb-6 text-2xl font-bold">Abonnés de {profile.nom}</h1>
