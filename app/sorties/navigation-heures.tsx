...
export default function NavigationHeures() {
  ...
  useEffect(() => {
    function changementJour(event: Event) {
      const evenement = event as CustomEvent<{
        date: string;
      }>;

      setJourActif(evenement.detail.date);
    }

    window.addEventListener("runin:jour-actif", changementJour);

    return () => {
      window.removeEventListener("runin:jour-actif", changementJour);
    };
  }, []);
...
const trouverJourActif = useCallback((conteneur: HTMLElement) => {
  ...
  function allerAHeure(heure: number) {
    ...
    const jour = trouverJourActif(conteneur);
    ...
    const lignes = Array.from(
      jour.querySelectorAll<HTMLElement>("[data-minute-depart]"),
    );
    ...
    const minuteCible = heure * 60;

    const cible = lignes.find(
      (ligne) => Number(ligne.dataset.minuteDepart) >= minuteCible,
    );
    ...
    if (cible) {
      ...
      conteneur.scrollTo({
        top: conteneur.scrollTop + cibleRect.top - conteneurRect.top,
        behavior: "smooth",
      });
    } else {
      ...
      const jourRect = jour.getBoundingClientRect();

      conteneur.scrollTo({
        top: conteneur.scrollTop + jourRect.bottom - conteneurRect.bottom,
        behavior: "smooth",
      });
    }
  ...
  useEffect(() => {
    ...
    function mettreAJour() {
      ...
      const premiereVisible = lignes.find(
        (ligne) => ligne.getBoundingClientRect().bottom > conteneurRect.top + 5,
      );
    ...
    conteneur.addEventListener("scroll", mettreAJour, {
      passive: true,
    });
