...
type ModifierSortieFormProps = {
  ...
  localisationInitiale: Localisation | null;
};

function versDatetimeLocal(dateISO: string) {
  const date = new Date(dateISO);

  const decalage = date.getTimezoneOffset() * 60 * 1000;
...
export default function ModifierSortieForm({
  ...
  const dureeInitiale = sortie.duree_estimee_minutes;

  const allureInitiale = sortie.allure_secondes_km;
  ...
  const [lieuDepart, setLieuDepart] = useState(sortie.lieu_depart);
  ...
  const [dateHeure, setDateHeure] = useState(
    versDatetimeLocal(sortie.date_heure_depart),
  );
  ...
  const [typeSortie, setTypeSortie] = useState(sortie.type_sortie);
  ...
  const [typeEntrainement, setTypeEntrainement] = useState(
    sortie.type_entrainement ?? "endurance_fondamentale",
  );

  const [distanceKm, setDistanceKm] = useState(
    sortie.distance_km !== null ? String(sortie.distance_km) : "",
  );

  const [denivelePositif, setDenivelePositif] = useState(
    sortie.denivele_positif_m !== null ? String(sortie.denivele_positif_m) : "",
  );

  const [dureeHeures, setDureeHeures] = useState(
    dureeInitiale !== null ? String(Math.floor(dureeInitiale / 60)) : "",
  );

  const [dureeMinutes, setDureeMinutes] = useState(
    dureeInitiale !== null ? String(dureeInitiale % 60) : "",
  );

  const [intensite, setIntensite] = useState(sortie.intensite ?? "moderee");

  const [allureMinutes, setAllureMinutes] = useState(
    allureInitiale !== null ? String(Math.floor(allureInitiale / 60)) : "",
  );

  const [allureSecondes, setAllureSecondes] = useState(
    allureInitiale !== null ? String(allureInitiale % 60) : "",
  );
  ...
  async function modifierSortie() {
    ...
    if (!dateHeure) {
      setMessage("Veuillez indiquer une date et une heure.");
      return;
    }
    ...
    const nombreMax = Number(nombreMaxParticipants);

    if (nombreMax < nombreParticipants || nombreMax > 100) {
      setMessage(
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        position_depart: `POINT(${localisation.longitude} ${localisation.latitude})`,

        date_heure_depart: new Date(dateHeure).toISOString(),
        ...
        denivele_positif_m: denivele,

        duree_estimee_minutes: dureeEstimeeMinutes,

        intensite: intensite,

        allure_secondes_km: allureSecondesKm,

        description: description.trim() || null,

        nombre_max_participants: nombreMax,

        genres_autorises: genresAutorises,
    ...
    if (error) {
      if (error.message.includes("GENRE_PARTICIPANT_DEJA_PRESENT")) {
        setMessage(
      ...
      if (error.message.includes("GENRE_ORGANISATEUR_REQUIS")) {
        setMessage("Votre propre genre doit rester autorisé.");
      ...
      console.error("Erreur modification sortie :", error);
