...
type Decision = "rejeter" | "infraction" | null;

type Sanction = "aucune" | "suspension" | "bannissement";
...
export default function ActionsSignalement({
  ...
  const [sanction, setSanction] = useState<Sanction>("aucune");
  ...
  const estAssigneAMoi =
    statut === "en_cours" && assigneA === utilisateurConnecteId;

  const estAssigneAutre =
    statut === "en_cours" && Boolean(assigneA) && !estAssigneAMoi;

  const estCompteCourant = cibleUtilisateurId === utilisateurConnecteId;

  const cibleProtegee =
    cibleUtilisateurRole === "administrateur" ||
    (roleConnecte === "moderateur" && cibleUtilisateurRole === "moderateur");
  ...
  const peutSanctionner =
    ...
    !cibleProtegee &&
    !sanctionActiveType &&
    (roleConnecte === "moderateur" || roleConnecte === "administrateur");
  ...
  async function prendreEnCharge() {
    ...
    if (error) {
      ...
      if (texte.includes("SIGNALEMENT_DEJA_PRIS_EN_CHARGE")) {
        ...
        console.error("Erreur prise en charge signalement :", error);
  ...
  async function traiter() {
    if (loading || !estAssigneAMoi || !decision) {
      return;
    }

    const commentaireNettoye = commentaire.trim();

    if (commentaireNettoye.length < 3 || commentaireNettoye.length > 1000) {
      setMessage("La justification doit contenir entre 3 et 1000 caractères.");
    ...
    if (
      ...
      if (!Number.isInteger(duree) || duree < 1 || duree > 365) {
        setMessage("La suspension doit durer entre 1 et 365 jours.");
    ...
    const sanctionFinale: Sanction = peutSanctionner ? sanction : "aucune";

    let confirmation = "Confirmer l'infraction sans appliquer de sanction ?";

    if (sanctionFinale === "suspension") {
      confirmation =
        `Suspendre ${cibleUtilisateurNom} ` +
        `pendant ${duree} jour(s) et traiter le signalement ?`;
    }

    if (sanctionFinale === "bannissement") {
      confirmation =
    ...
    const { error } = await supabase.rpc(
      "admin_traiter_signalement_avec_sanction",
      {
        ...
        p_sanction: sanctionFinale,

        p_duree_jours: sanctionFinale === "suspension" ? duree : null,
      },
    );
  ...
  function gererErreur(error: {
    ...
    if (texte.includes("SIGNALEMENT_DEJA_CLOTURE")) {
      setMessage("Ce signalement a déjà été clôturé.");
    ...
    if (texte.includes("SANCTION_DEJA_ACTIVE")) {
      setMessage("Cet utilisateur possède déjà une sanction active.");
    ...
    if (texte.includes("AUTO_SANCTION_INTERDITE")) {
      setMessage("Vous ne pouvez pas sanctionner votre propre compte.");

      return;
    }

    if (texte.includes("CIBLE_HIERARCHIQUE_PROTEGEE")) {
      setMessage("Votre rôle ne permet pas de sanctionner cet utilisateur.");
    ...
    setMessage("Impossible de traiter le signalement.");
  }
      ...
      <section className="rounded-xl border p-5">
        ...
        <div className="mt-4 rounded-lg border p-4">
          <p className="font-medium">
            {statut === "rejete" ? "Signalement rejeté" : "Signalement traité"}
          </p>

          <p className="mt-1 text-sm text-muted-foreground">
            {afficherDate(traiteAt)}

            {traiteParNom ? ` par ${traiteParNom}` : ""}
          </p>
  ...
  return (
    <section className="rounded-xl border p-5">
      ...
      <div className="mt-5">
        {statut === "ouvert" && (
          <>
            <p className="mb-3 text-sm text-muted-foreground">
              Prenez en charge le signalement pour pouvoir rendre une décision.
            </p>

            <Button type="button" onClick={prendreEnCharge} disabled={loading}>
        ...
        {estAssigneAutre && (
          <p className="text-sm text-muted-foreground">
            Signalement pris en charge par{" "}
      ...
      <fieldset
        disabled={!estAssigneAMoi || loading}
        className={`
          mt-6
          border-t
          pt-6
          transition-opacity
          ${estAssigneAMoi ? "opacity-100" : "opacity-35"}
        `}
      >
        {/* DECISION */}

        <p className="text-sm font-semibold">Décision</p>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <button
            ...
            className={`
              ...
              text-left
              transition
              ${decision === "rejeter" ? "border-border bg-muted" : ""}
            `}
          >
            <span className="font-medium">Rejeter le signalement</span>
          ...
          <button
            ...
            `}
          >
            <span className="font-medium">Confirmer l&apos;infraction</span>

            <span className="mt-1 block text-sm text-muted-foreground">
              Le contenu ou le comportement enfreint les règles.
            </span>
          </button>
        </div>
              ...
              <div className="mt-3 space-y-3">
                <label className="flex items-center gap-3 text-sm">
                  <input
                    type="radio"
                    name="sanction"
                    checked={sanction === "aucune"}
                    onChange={() => setSanction("aucune")}
                  />
                  Aucune sanction
                </label>
                ...
                <label className="flex items-center gap-3 text-sm">
                  <input
                    type="radio"
                    name="sanction"
                    checked={sanction === "suspension"}
                    onChange={() => setSanction("suspension")}
                  />
                ...
                <label className="flex items-center gap-3 text-sm">
                  <input
                    type="radio"
                    name="sanction"
                    checked={sanction === "bannissement"}
                    onChange={() => setSanction("bannissement")}
                  />
          ...
          <div className="mt-6">
            ...
            >
              {decision === "rejeter"
                ...
                : sanction === "aucune" || !peutSanctionner
        ...
        <div className="mt-6 border-t pt-5">
          <Button
            ...
            className="px-5"
          >
            {loading ? "Traitement..." : "Traiter le signalement"}
          </Button>
        </div>
      </fieldset>
