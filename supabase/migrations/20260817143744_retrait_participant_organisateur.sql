-- ============================================================
-- 1. Nouveau statut : revoquee
-- ============================================================

alter table public.demandes_participation
drop constraint if exists demandes_participation_statut_check;

alter table public.demandes_participation
add constraint demandes_participation_statut_check
check (
    statut in (
        'en_attente',
        'acceptee',
...
-- ============================================================
-- 3. Une désinscription volontaire doit prévenir l'organisateur
--    seulement si le participant se retire lui-même.
--
--    Si c'est l'organisateur qui retire le participant,
--    la RPC ci-dessous créera une notification différente.
-- ============================================================

drop trigger if exists
trigger_notification_desinscription_participant
on public.participations;

create trigger trigger_notification_desinscription_participant
after delete on public.participations
for each row
when (auth.uid() = old.utilisateur_id)
execute function public.notifier_desinscription_participant();


-- ============================================================
-- 4. RPC : retirer un participant
-- ============================================================
...
begin
    ...
    -- Vérification MFA si le compte utilise le MFA
    ...
    -- --------------------------------------------------------
    -- Si cette participation venait d'une demande acceptée,
    -- on marque la dernière demande acceptée comme révoquée.
    -- --------------------------------------------------------
...
-- Empêche l'utilisateur retiré de se réinscrire
-- ou de refaire une demande.
...
do nothing;
    ...
    -- --------------------------------------------------------
    -- Retire réellement la participation
    -- --------------------------------------------------------
    ...
    -- --------------------------------------------------------
    -- Notification destinée au participant retiré
    -- --------------------------------------------------------
